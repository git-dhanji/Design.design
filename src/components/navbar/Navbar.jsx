    ...
    <div>
      ...
      >
        <div className="mx-auto flex justify-between items-center max-w-screen-xl w-[1280px] text-[#f1f1f1]">
          ...
          <div className="links font-['Ubuntu']  md:gap-6 lg:gap-10    hidden md:flex">
            {listofurl &&
              listofurl?.map((item, key) => (
                <a
                  key={key}
                  href={item.loction}
                  className={`text-md text-white font-[200] capitalize cursor-pointer ${key == listofurl.length - 1 && "ml-32 md:ml-10"}`}
                >
                  {item.name}
                </a>
              ))}
